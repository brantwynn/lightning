...
class FileUpload extends EntityFormProxy {
   ...
   *   The token replacement service.
   */
  public function __construct(array $configuration, $plugin_id, $plugin_definition, EventDispatcherInterface $event_dispatcher, EntityManagerInterface $entity_manager, BundleResolverInterface $bundle_resolver, AccountInterface $current_user, Token $token) {
  ...
  protected function generateEntity($input) {
    ...
    $file = file_move($this->getFile($entity), $this->getPermanentUri($entity));
    $file->setTemporary();
    $file->save();
